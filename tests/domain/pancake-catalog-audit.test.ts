...
test("catalog audit emits aggregate source coverage without raw product content", async () => {
      ...
      data: [
        {
          ...
          images: ["https://cdn.example.test/catalog/123456/photo-a.jpg?token=IMAGE_QUERY_SECRET"],
          product: {
            id: "product-a",
            name: "RAW_PRODUCT_NAME_SECRET",
            note: "PRIVATE_NOTE_SECRET",
            note_product: "APPROVED_DESCRIPTION_SECRET",
            image: "https://cdn.example.test/catalog/123456/hero-a.webp",
            categories: [{ id: 10, name: "RAW_CATEGORY_NAME_SECRET" }],
          },
          variations_warehouses: [{ remain_quantity: 999999 }],
        },
        {
          id: "variation-b",
          product_id: "product-a",
          images: ["http://legacy.example.test/images/photo-b.png"],
          product: {
            id: "product-a",
            name: "RAW_PRODUCT_NAME_SECRET",
            note: "PRIVATE_NOTE_SECRET",
      ...
      data: [
        {
          ...
          product: {
            id: "product-b",
            name: "SECOND_RAW_PRODUCT_NAME_SECRET",
            note: "SECOND_PRIVATE_NOTE_SECRET",
            note_product: "   ",
            image: null,
            categories: [],
          },
  ...
  const client = {
    async getJson(endpoint: string, query?: Readonly<Record<string, string | number | boolean>>) {
      requested.push({ endpoint, query });
      if (endpoint.endsWith("/categories")) {
        return {
          success: true,
          data: [
            { id: 10, name: "Shirts", nodes: [] },
            { id: 11, name: "shirts", nodes: [] },
          ],
        };
      }
  ...
  assert.deepEqual(report.products, {
    total: 2,
    withNoteProduct: 1,
    withoutNoteProduct: 1,
    noteProductCoveragePercent: 50,
    malformedNoteProductCount: 0,
    withCategoryAssignments: 1,
    categoryAssignmentCoveragePercent: 50,
  });
  ...
  assert.deepEqual(report.images.origins, [
    {
      ...
      referenceCount: 1,
      pathShapes: ["/images/:file.png"],
    },
    {
      scheme: "https",
      hostname: "cdn.example.test",
      referenceCount: 2,
      pathShapes: ["/catalog/:id/:file.jpg", "/catalog/:id/:file.webp"],
    },
  ]);
  assert.equal(report.images.malformedCount, 1);
  assert.equal(report.images.credentialBearingCount, 0);
  assert.deepEqual(report.categories, {
    count: 2,
    rootCount: 2,
    maxDepth: 1,
    duplicateNormalizedNameCount: 1,
    duplicateIdCount: 0,
    assignedProductCount: 1,
    knownAssignmentReferenceCount: 1,
    unknownAssignmentReferenceCount: 0,
    classification: "partial",
  });
  assert.deepEqual(
    requested.map(({ endpoint }) => endpoint),
    [
      "/shops/4741464/products/variations",
      "/shops/4741464/products/variations",
      "/shops/4741464/categories",
    ],
  );
  ...
  for (const forbidden of [
    "RAW_PRODUCT_NAME_SECRET",
    "PRIVATE_NOTE_SECRET",
    "APPROVED_DESCRIPTION_SECRET",
    "RAW_CATEGORY_NAME_SECRET",
    "IMAGE_QUERY_SECRET",
    "999999",
  ]) {
    assert.equal(serialized.includes(forbidden), false, `${forbidden} must not escape the audit boundary`);
  }
});
        ...
        data: [
          {
            id: "variation-a",
            product_id: "product-a",
            images: ["https://user:password@cdn.example.test/catalog/a.jpg"],
            product: {
              id: "product-a",
              note_product: null,
              image: "https://cdn.example.test/catalog/a.jpg",
              categories: [],
            },
          },
        ],
...
test("catalog audit fails on inconsistent repeated product source facts", async () => {
        ...
        data: [
          {
            ...
            product_id: "product-a",
            images: [],
            product: { id: "product-a", note_product: "A", image: null, categories: [] },
          },
          {
            id: "variation-b",
            product_id: "product-a",
            images: [],
            product: { id: "product-a", note_product: "B", image: null, categories: [] },
          },
        ],
      };
    },
  ...
  await assert.rejects(
    () => runPancakeCatalogAudit({ client, shopId: 4741464 }),
    /inconsistent repeated product source facts/i,
  );
});
...
test("trusted-local catalog audit refuses CI before reading Pancake credentials", () => {
  ...
  const result = spawnSync(
    process.execPath,
    ["--experimental-strip-types", "scripts/pancake-catalog-audit.ts"],
    {
      cwd: process.cwd(),
      encoding: "utf8",
      env: {
