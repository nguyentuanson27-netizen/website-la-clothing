...
Status: **WAVES 0–4 runtime implementation is delivered through U24; Checkpoint A PASS, Checkpoint B PASS, and Checkpoint D PASS. U9/M1 operational closure is GREEN (executed on `84c99db3de6757c3ded4396644eb4dae25869e09`, tree `ac2e395edafaf5acc83fe98c632145ef7b084aa3`). Wave 5 is partly delivered: U25/M3 is implemented (standalone Merchant mapper + ADR 0007 O3 runtime, nothing published, O2 still open); U26 (PR #198) and U27 are now implemented and Checkpoint E is PASSED; Merchant↔JSON-LD parity is proved for identity, grouping, MPN, URL, price and the resolvable availability domain, but the feed↔JSON-LD consistency launch gate stays OPEN on one reachable availability divergence. U28 retains its own prerequisites. O2 stays open, so nothing is published to Merchant.**
...
- [ ] Before Merchant/index launch, prove feed vs JSON-LD identity/price/availability consistency. **Partly proved, gate still OPEN.** `tests/domain/merchant-structured-data-parity.test.ts` proves variation identity, `item_group_id`/`productGroupID`, ADR 0008 MPN, exact U12 URL and exact promotion-aware price MATCH, that availability semantics and the publishable standalone set MATCH across the whole resolvable stock domain (no rows, zero, positive), and that unresolved-price/duplicate-MPN/composite/unaddressable candidates fail closed compatibly. **The gate does not close** while one reachable divergence stands: a negative mirrored warehouse quantity is `AVAILABILITY_UNRESOLVED` to the feed (offer omitted) but sold out to the PDP, so U27 publishes an exact `OutOfStock` Offer for it. Closing requires either (1) a scoped runtime reconciliation, or (2) an explicit owner acceptance that omission-vs-`OutOfStock` is compatible, with this gate's normative wording updated to say so. Neither has happened. See `docs/audits/merchant-jsonld-parity.md`.
