...
Status: **WAVES 0–4 runtime implementation is delivered through U24; Checkpoint A PASS, Checkpoint B PASS, and Checkpoint D PASS. U9/M1 operational closure is GREEN (executed on `84c99db3de6757c3ded4396644eb4dae25869e09`, tree `ac2e395edafaf5acc83fe98c632145ef7b084aa3`). Wave 5 is partly delivered: U25/M3 is implemented; U26 (PR #198) and U27 are implemented and Checkpoint E is PASSED; U27a closed the availability divergence. O2 is now owner-approved as Vietnam / `vi` / `VND`, but M3/M4 runtime still has no reviewed trusted server-owned market authority, so the public Merchant route remains fail-closed until that implementation lands. The Merchant↔JSON-LD one-survivor contract is also owner-approved, while its U27 implementation/parity proof remains open. U28 retains its own O4/GTM prerequisites.**
...
- [x] **U4** #152 W2a — prove collision-safe metadata uniqueness replacement before slug/path cleanup. *(PR-A4 / #159, merged)* **U4 delivered; B5 is now resolved. U29/W2b is owner-unblocked but still unimplemented; slug/path copy remains until pair-level publish enforcement + real-catalog verification pass.**
...
- [ ] Before Merchant/index launch, prove feed vs JSON-LD identity/price/availability consistency. **Availability convergence closed by U27a. The remaining one-survivor contract is now RESOLVED, but its U27 implementation + focused parity verification are still pending.** `tests/domain/merchant-structured-data-parity.test.ts` already proves variation identity, `item_group_id`/`productGroupID`, ADR 0008 MPN, exact U12 URL and exact promotion-aware price MATCH for variant-family states, that availability semantics and the publishable standalone set MATCH across the whole resolvable stock domain (no rows, zero, positive), and that unresolved-price/duplicate-MPN/composite/unaddressable candidates fail closed compatibly. U27a closed negative-row availability and exclusion-ordering mismatches.

  **Approved one-survivor contract:** when filtering/exclusion leaves exactly one publishable standalone sibling, Merchant continues to publish that exact survivor. U27 must emit a standalone `Product` representing that same survivor, using the same U12 variant deep-link and verified variant facts; U27 must not emit a one-member `ProductGroup`. The gate remains open until the dedicated implementation PR proves exact variation identity, URL, MPN, price and availability parity in that state. See `docs/audits/merchant-jsonld-parity.md` and `docs/specs/la-clothing-owner-approved-facts-and-decisions.md`.
...
- [ ] **U29** #152 W2b — metadata cleanup after U4 uniqueness proof. **B5 is RESOLVED — owner-unblocked, implementation open.** Implement pair-level `(seoTitle, seoDescription)` uniqueness among published products; drafts may be missing/duplicate and admin should warn on collision; publish requires both fields and blocks pair collision. Keep the current slug/path discriminator until enforcement + real-catalog verification prove slug-free copy collision-free.
...
- [ ] **U32** product-level remainder of #152 W5 + W6 — verified product-level identifiers/attributes + Organization only; **do not redefine ProductGroup/variant Offer owned by U27**. B2 is resolved, so Organization enrichment is owner-unblocked; U32 remains open because U32b itself is not implemented. Substates below.
  ...
  - [ ] **U32b** W6 Organization enrichment (`logo`, `sameAs`, `contactPoint`, `address`) — **B2 RESOLVED; owner-unblocked, implementation open.** Use only approved contact/legal/social facts from the normative owner source; do not infer additional Organization facts.
- [ ] **U33** #152 P5/W13 — evergreen pages only from human-approved facts; no invented policies. **B1–B4 and B6 are RESOLVED; U33 is owner-unblocked but not implemented.** Build About/Returns/Shipping/Size/Contact and approved policy surfaces from one shared fact authority; founding story/values remain optional/open and must not be invented.
...
| **B5** | **RESOLVED** — pair-level `(seoTitle, seoDescription)` uniqueness among published products; drafts may be missing/duplicate; collision blocks publish | Repository owner / brand authority | U29/W2b owner-unblocked | Do not remove slug/path discriminator until enforcement + real-catalog verification pass |
...
- [ ] Feed↔JSON-LD one-survivor parity implementation + verification green under the approved contract.
