...
- [x] Before Merchant/index launch, prove feed vs JSON-LD identity/price/availability consistency. **Proved** by `tests/domain/merchant-structured-data-parity.test.ts`: variation identity, `item_group_id`/`productGroupID`, ADR 0008 MPN, exact U12 URL, exact promotion-aware price, availability semantics and the publishable standalone set all match, and unresolved/duplicate/composite/unaddressable cases fail closed compatibly. One bounded difference is recorded rather than equalized — a negative mirrored warehouse quantity is `AVAILABILITY_UNRESOLVED` to the feed but sold out to the PDP and therefore to U27. See `docs/audits/merchant-jsonld-parity.md`; closing it needs an owner decision, not a verification PR.
