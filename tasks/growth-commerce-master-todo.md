...
- [ ] **U34** #152 P6/W16/W17 — SEO admin/operational readiness; advisory UI does not become hard unreviewed policy. Substates below.
  - [x] **U34a** W16 advisory SEO length guidance in the product editor. *(`src/commerce/seo-length-guidance.ts` holds the advisory targets — 60 for `seoTitle`, 155 for `seoDescription` — and `SeoLengthField` counts towards them live, announcing politely, describing the input via `aria-describedby`. Advice only: the enforced bounds stay `PRODUCT_CONTENT_LIMITS` (500/2000), nothing is disabled or gated, and no publish gate exists. A domain test pins every advisory target strictly below its enforced bound so the advice cannot become a limit; the admin browser spec submits a 61-character title through the real form and server action and asserts it persists exactly. B5 metadata uniqueness untouched.)*
  - [ ] **U34b** W17 SEO/editorial health filters (`missing-seo`, `missing-editorial`) — server-side bounded filter reusing the admin product listing/query authority, with an explicit query-param allowlist. No health score, no client-side load-all-and-filter.
