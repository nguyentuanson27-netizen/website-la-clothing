...
- [x] **U26** #153 M4 + #151 — bounded public feed/cache/single-flight/backoff + durable promotion revision; no request-controlled cache dimensions. **Implemented** (PR #198, merged as `2d5ea84045f61fc1249076379dd0816d37499546`). Checkpoint E verified on exact head `1d003dc4d917c138a2c12f93c98b4a38be487754`. O2 remains open, so the production route still fails closed with a bounded `503`.
...
- [ ] Before Merchant/index launch, prove feed vs JSON-LD identity/price/availability consistency. **Availability convergence closed by U27a; gate still OPEN on the family-collapse contract.** `tests/domain/merchant-structured-data-parity.test.ts` proves variation identity, `item_group_id`/`productGroupID`, ADR 0008 MPN, exact U12 URL and exact promotion-aware price MATCH, that availability semantics and the publishable standalone set MATCH across the whole resolvable stock domain (no rows, zero, positive), and that unresolved-price/duplicate-MPN/composite/unaddressable candidates fail closed compatibly. The one reachable divergence PR #199 recorded — a negative mirrored warehouse quantity, `AVAILABILITY_UNRESOLVED` to the feed but ordinary arithmetic to the PDP — is **closed by U27a** through route (1), a scoped runtime reconciliation. A server-only per-variant availability-resolution signal now travels from the catalog read to the U27 boundary, so any variant with a malformed mirrored row is omitted by both consumers instead of being published as an exact availability claim. The rule is per row, not on the total: `[5, -3]`, `[3, -3]` and `[100, -1]` are unresolved as surely as `[-3]`, and the product-level fallback offer is derived only from variants whose inventory can state an availability, so a suppressed claim cannot reappear one node up. Shopper-facing PDP stock behaviour is unchanged, Merchant is unchanged, and no database query was added.

  **What still holds the gate open (the remaining known gap, not a proof of exhaustion — two review passes each found a further mismatch):** when exclusion leaves a single publishable sibling, U27's existing rules collapse the family to a product-level `Product`, so Merchant publishes an exact variant offer while JSON-LD publishes no exact *variant* identity for it. The two statements agree on product, price and availability and neither is false, but the publishable exact-variant sets are not equal in that state. That is the pre-existing single-variant-family format rule rather than anything U27a introduced, and reconciling it means either changing U27's ProductGroup eligibility or omitting the survivor from Merchant — an authority decision, not a bug fix. See `docs/audits/merchant-jsonld-parity.md`.
...
- [ ] **U37** #152 P6/W21 — catalog URL-volume trigger before sitemap hard cliff; shard only when evidence warrants it. Capacity audit recorded in `docs/audits/sitemap-capacity-w21.md`. *(Contract verified from source: 4 static canonical paths + `MAX_DYNAMIC_SITEMAP_PATHS` = a 50,000-URL single-sitemap budget, where dynamic = current present/active products for the configured shop plus **all** published website-owned collections. Crossing it is an HTTP 500, not a degraded sitemap. Two answers, deliberately separate. **Runtime action: NO SHARDING NOW** — an operational choice under the current indexing-disabled state, not a proven numerical headroom result: production runs `SEARCH_INDEXING_ENABLED=false`, enforced in code via `TEMPORARY_PRODUCTION_HOSTS`, so `/sitemap.xml` emits 0 URLs and the throwing path never executes. **Capacity verdict: INSUFFICIENT EVIDENCE** — no attributable production count exists under the sitemap predicate. The product artifacts (`pricing-evidence-w3.md`: 42 current products from the Pancake source API; `merchant-identity-m1.md`: 83 mirrored products, predicate unrecorded) do not measure it, and published collections have no evidence and no cap — `MAX_COLLECTION_LIST` bounds a list query and a membership array, not how many collections may exist — so no utilization, headroom or growth multiple is claimed. `pnpm sitemap:capacity:audit` is the new repeatable authority: aggregate `COUNT`s only, sharing one predicate with `listCanonicalPaths` — proven by a count/list parity test — emitting no slug, URL or catalog content, and encoding no threshold. **Still open, and all three are Gate S preconditions — they gate organic indexing enablement rather than following it:** an authorized operator must run the audit against production and record the block, owner/cadence/trigger values must be agreed, and — because a historical baseline does not describe capacity at activation while recurring monitoring only starts after enablement — the audit must be **rerun on the exact activation head immediately before indexing is enabled**, be within the hard budget **and not satisfy the approved Act trigger** — the hard bound is a failure boundary, not a release threshold, so an approved Act(Y) being true blocks Gate S and requires U37b first. That result is pinned to data state as well as code SHA: any product-mirror sync or collection publication mutation after the run invalidates it and forces a rerun. Whether a Warning blocks is part of D3's owner approval (`BLOCK` | `ALLOW_WITH_ACK`), not a default this audit sets. All three before Gate S proceeds, since the first request after enablement goes straight through the budget guard. `docs/operations/p12-search-exposure.md` now carries that precondition at the point of action. `src/app/sitemap.ts` HTTP behaviour is unchanged; U31 `lastModified`, indexing enablement and image sitemap are untouched.)*
