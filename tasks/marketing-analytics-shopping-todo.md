...
Status: **PR-A (T1–T3), T4, T5, T6, M2, T7 (U24 / PR #193), M1 (PR #175 + PR #194 + operational closure), M3 (U25), and M4 (U26 / PR #198) IMPLEMENTED; Checkpoint D PASSED and Checkpoint E PASSED. Merchant↔variant JSON-LD parity is proved, and the feed↔JSON-LD consistency launch gate is PASSED after U27a closed the last availability divergence. T8 and M5/V1 remain proposed and require human approval of `tasks/marketing-analytics-shopping-plan.md` before `/build`.**
...
- [x] **U27a:** any malformed mirrored warehouse row makes a variant's availability unstatable, and both consumers omit it. The rule is per row, not on the total, so `[5, -3]`, `[3, -3]` and `[100, -1]` are unresolved as surely as `[-3]`. Carried by a server-only `variantAvailabilityResolvedById` from the catalog read to the U27 boundary; no extra DB query, no client contract widening, shopper-facing PDP stock behaviour and Merchant both unchanged.
...
- [x] **Convergence launch gate** (`Before Merchant/index launch, prove feed vs JSON-LD ... consistency` in `tasks/growth-commerce-master-todo.md`) is **PASS**, closed by U27a's scoped runtime reconciliation rather than by accepting the divergence. Evidence in `docs/audits/merchant-jsonld-parity.md`.
