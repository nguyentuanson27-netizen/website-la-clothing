...
T8, M5 and V1 are **not** implemented. No GTM loader exists: T8 still owns the first actual GTM load and CSP opening. **M3 (U25) is implemented** as the standalone Merchant mapper/O3 runtime, and **M4 (U26 / PR #198) adds the bounded public delivery layer**: GET `/feeds/google-merchant`, RSS serialization, whole-generation query/offer/byte envelopes, complete-success cache, process-local single-flight, and fixed-key failure backoff. **O2 is owner-approved as Vietnam / `vi` / `VND`, but the reviewed trusted server-owned runtime authority has not yet been implemented, so the production route still fails closed with bounded `503`.** M3/M4 accept no caller/request market authority; that remains true when the approved market is wired.
...
*(Implemented on the U25 branch; `src/commerce/merchant-offer-mapper.ts` + `merchant-offer-repository.ts` + `merchant-apparel-facts.ts` + `product-merchant-facts-{admin,repository}.ts`. O2 owner decision is resolved; trusted runtime market authority remains a separate implementation prerequisite.)*
...
- [x] **O2 owner decision is resolved without becoming caller-controlled runtime configuration.** Approved market is Vietnam / `vi` / `VND`, but the current mapper still reports `market: UNRESOLVED` and `activationBlockedReasons: ["MERCHANT_MARKET_UNRESOLVED"]` until a reviewed trusted server-owned configuration source is wired. It emits `priceVnd` rather than currency-qualified Merchant `price`, accepts no market argument, and ignores an extra caller-supplied syntax-valid market object.
...
*(Implemented on U26 / PR #198; O2 owner decision is resolved, but trusted runtime market authority is not yet wired, so runtime delivery still fails closed with `503`.)*
