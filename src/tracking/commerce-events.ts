 ...
 *
 * `value` is derived from the normalized immutable item projection. The immutable snapshot's
 * merchandise total is also checked against that sum so an inconsistent first-party snapshot fails
 * closed instead of silently publishing whichever side happened to be chosen. Shipping remains
 * separate; the order total is carried as a first-party fact for destinations whose owner-approved
 * semantics use it.
 */
export function buildPurchaseEvent(facts: CommercePurchaseFacts): PurchaseEvent {
  ...
  const suppliedMerchandiseValueVnd = requireIntegerVnd(facts.merchandiseValueVnd, "merchandise value");
  const merchandiseValueVnd = sumMerchandiseVnd(items);
  if (suppliedMerchandiseValueVnd !== merchandiseValueVnd) {
    throw new RangeError("merchandise value must match the canonical item sum");
  }
