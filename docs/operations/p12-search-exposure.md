...
- **U37 / W21 is a precondition of that gate, in full.** Gate S must not enable indexing until all three are closed: (1) a **fresh** attributable production capacity audit, run on the exact activation head immediately before enablement, is within budget; (2) a named monitoring owner is approved; (3) cadence plus warning/act trigger values are approved. See `docs/audits/sitemap-capacity-w21.md` §5 and §8. If any item remains open, indexing remains disabled.
  - The capacity run is `DATABASE_URL=<production> PANCAKE_SHOP_ID=<shop> pnpm sitemap:capacity:audit`, recorded with exact SHA, environment, timestamp, the reported figures and the mirror-freshness line (`CatalogSyncState.syncedAt` / `updatedAt` for the shop). A historical block establishes a baseline but does **not** authorize a later enablement — the catalog can grow in between, and recurring monitoring does not begin until indexing is on.
  - **The activation-time capacity result is valid only while sitemap-eligible catalog state remains unchanged.** Any product-mirror sync that changes `isPresent`, `isActive` or shop membership, or any collection publish/unpublish/create/delete that changes the published-collection count, invalidates it and requires a fresh rerun before indexing may be enabled. The git SHA pins which predicate ran, not which rows it counted.
  - The fresh activation-time run must be within the hard budget **and must not satisfy the approved Act trigger**. Within-budget is not on its own sufficient: the hard bound is a failure boundary, not a release threshold, and enabling at zero headroom means one new product or published collection turns `/sitemap.xml` into a 500.
  - Branches, evaluated in order: over the hard budget → do not enable, open U37b and shard first; approved **Act** trigger already true → **do not enable**, U37b must be implemented first; approved **Warning** true but Act false → follow the owner-approved D3 warning behaviour (`BLOCK` → do not enable, open or advance U37b; `ALLOW_WITH_ACK` → open the U37b plan, record the owner's explicit acknowledgement beside the activation block, then continue); owner/cadence/trigger not yet approved → **still do not enable**, regardless of the count; all three preconditions closed, no trigger true, and the other Gate S requirements satisfied → human approval may proceed.
  - The sitemap is a single document with a hard bound and no partial/index fallback, so enabling over an already-oversized catalog publishes an HTTP 500 at `/sitemap.xml` on the first request. Neither a partial sitemap nor raising the per-document bound is an acceptable way past this gate.
