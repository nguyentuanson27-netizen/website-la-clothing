...
- **Sitemap capacity is a precondition of that gate (#152 W21 / U37).** Before indexing is enabled, an authorized operator must run `pnpm sitemap:capacity:audit` against the production database, record the attributable block in `docs/audits/sitemap-capacity-w21.md`, and confirm `exceedsDynamicBudget` is `false`. The sitemap is a single document with a hard bound and no partial/index fallback, so enabling indexing over an already-oversized catalog publishes an HTTP 500 at `/sitemap.xml` on the first request. If the bound is already exceeded, shard first.
