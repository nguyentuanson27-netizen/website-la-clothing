...
- **U37 / W21 is a precondition of that gate, in full.** Gate S must not enable indexing until all three are closed: (1) a **fresh** attributable production capacity audit, run on the exact activation head immediately before enablement, is within budget; (2) a named monitoring owner is approved; (3) cadence plus warning/act trigger values are approved. See `docs/audits/sitemap-capacity-w21.md` §5 and §8. If any item remains open, indexing remains disabled.
  ...
  - Branches: capacity over budget → do not enable, open U37b and shard first; capacity within budget but owner/cadence/trigger not yet approved → **still do not enable**; all three closed and the other Gate S requirements satisfied → human approval may proceed.
